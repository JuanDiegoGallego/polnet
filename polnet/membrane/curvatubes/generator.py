...
from polnet.membrane.curvatubes.utils import save_nii, slices, project_average
from polnet.membrane.curvatubes.filters import GeneralGaussianBlur3D_periodic, GeneralGaussianBlur3D_notperiodic
from polnet.membrane.curvatubes.filters import my_custom_GradHess

from polnet.membrane.curvatubes.energy import polykap_deg2, ratio_discr


def _generate_shape(params, tomo_shape, delta_x, xi, optim_method, optim_props,
                    flow_type, mode, M0 = None,
                   snapshot_folder = '', exp_title = '',
                   cond_take_snapshot = None, display_all = False,
                   return_var = False, return_energy = False, check_viable = False, verbose=False) :
    ...
    Z, X, Y =  tomo_shape
    v0 = 40 * delta_x * np.random.rand(3, Z, X, Y)
    ...
    if flow_type == 'cons' :
        from polnet.membrane.curvatubes.filters import my_custom_Div
