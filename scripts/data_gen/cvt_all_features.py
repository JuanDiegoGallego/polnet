"""
Script for generating tomograms simulating curvatubes-generated membranes
annd proteins
    Input:
...
__author__ = 'Juan Diego Gallego Nicolás'
...
VOI_SHAPE = (500,500,250)#(500, 500, 250)
VOI_OFFS  =  ((4,496),(4,496),(4,246))#((4,496),(4,496),(4,246))
#VOI_SHAPE = (500,500,250)#(500, 500, 250)
#VOI_OFFS  =  ((4,496),(4,496),(4,246))#((4,496),(4,496),(4,246))
...
"""
    Curvatubes parameters
"""
cvt_allParams = [
    [0.02, 1, 1.1, 1, 0, 0, 0, -0.3],
    [0.02, 1, 1.2, 1, 0, 0, 0, -0.3],
]
...
tomod_id = sys.argv[1]
tomod_id = int(tomod_id)

cvt_params = cvt_allParams[tomod_id]
...
# OUTPUT FILES
OUT_DIR = os.path.realpath(ROOT_PATH + '/data_generated/all_v4/Tomo0' + str(tomod_id)) # '/out_all_tomos_9-10' # '/only_actin' # '/out_rotations'
...
# clean_dir(TEM_DIR)
# clean_dir(TOMOS_DIR) TODO Comentado para no borrar
...
for p_id, p_file in enumerate(MEMBRANES_LIST):
    ...
    elif memb.get_type() == 'curvatubes':
        mb_cvt_generator = CvtGen(cvt_params=cvt_params) # TODO Don't caring about the .mbs file
        set_mbs = SetMembranes(voi, VOI_VSIZE, mb_cvt_generator, param_rg, memb.get_thick_rg(), memb.get_layer_s_rg(),
                               hold_occ, memb.get_over_tol(), bg_voi=bg_voi)
        set_mbs.build_set(verbosity=True)
        hold_den = set_mbs.get_tomo()
        if memb.get_den_cf_rg() is not None:
            hold_den *= mb_cvt_generator.gen_den_cf(memb.get_den_cf_rg()[0], memb.get_den_cf_rg()[1])
    ...
    entity_id += 1
    # Save membrane lbls file
    tomo_mem_lbls_out = TOMOS_DIR + '/tomo_mem_lbls_' + str(tomod_id) + '.mrc'
    lio.write_mrc(tomo_lbls, tomo_mem_lbls_out, v_size=VOI_VSIZE)
